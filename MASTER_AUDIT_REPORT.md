...
### Phase 10 — complete (2026-07-27)

Implements the "Sri Lankan Journey" design language to elevate the UI. Replaced the static hero image with a cinematic 3-image crossfading slider (with a glassmorphic pause control). Upgraded the mobile sidebar with staggered slide-in link animations and glassmorphism. Added a scroll-driven SVG "journey line" to the home page that physically draws itself as the user scrolls. Modernized form and gallery transitions using the View Transitions API for seamless crossfades, while respecting `prefers-reduced-motion` globally. Updated the logo asset and optimized its responsive sizing. Finally, refined and simplified the entire global color palette (both Light and Dark themes) to define clearer roles, removing overly warm secondary backgrounds in favor of neutral tones and green-tinted charcoal for better brand cohesion and contrast.

### Phase 11 — complete (2026-07-27)

Implemented rigorous structural accessibility upgrades across the site based on the original audit's findings. A global hidden "Skip to main content" link was added to `BaseLayout` and paired with `<main id="main-content">` anchors on all nine pages for reliable keyboard navigation. Replaced browser default focus rings with clear, robust, accessible `:focus-visible` outlines in the global styles. Lastly, thoroughly hardened form accessibility across `contact.astro`, `inquiry.astro`, and the feedback form in `index.astro` by ensuring every interactive input, select, and textarea possesses a well-associated `<label>` and explicitly includes `aria-required="true"` on mandatory fields.
